...
- [ ] **Step 1: Generate a branded 1200×630 image**
...
- [ ] **Step 2: Point both cards at it, with true dimensions**
...
- [ ] **Step 4: Reconcile the descriptions — one source string**
...
- [ ] **Step 5: Gate** — build, tsc, lint clean.
...
- [ ] **Step 2: `public/robots.txt`** — allow all, and reference the sitemap absolutely
...
- [ ] **Step 4: Verify all three are copied into `out/`**
...
- [ ] **Step 1: One `@graph`, built from `DATA`**
...
- [ ] **Step 2: Include `url` on an article ONLY where `DATA` identifies that article**
...
- [ ] **Step 3: Gate** — build clean; the block parses.
...
- [ ] **Step 1: Skip link** — first focusable element in the document, visually hidden until
focused, targeting the main content. `<main>` needs a matching `id` and `tabIndex={-1}`.

- [ ] **Step 2: `<header>` landmark** around the hero region.

- [ ] **Step 3: Real `<nav>` landmarks** for the social rail and mobile dock. Adding `aria-label`
...
- [ ] **Step 4: Icon-only links get real accessible names.** WhatsApp and phone in **both**
`hero.tsx` and `contact.tsx` currently rely on `title` alone. Add `aria-label`.

- [ ] **Step 5: Custom SVGs** in `components/icons/index.tsx` have neither a name nor
...
- [ ] **Step 6: Name every section** via `aria-labelledby` pointing at its existing heading.
Give each heading a stable `id`.

- [ ] **Step 7: Eliminate the horizontal overflow**
...
- [ ] **Step 8: Delete the dead classes §9 assigns to this phase**
...
- [ ] **Step 1: Exact relationships, not "consistency"**
...
- [ ] **Step 2: Exactly one of each** — no duplicate `title`, `description`, `canonical` or
`robots` tag. The metadata skill's priority-1 rule.

- [ ] **Step 3: The social card, checked end to end**
...
- [ ] **Step 4: Artefacts, with status codes**
...
- [ ] **Step 5: JSON-LD, validated not counted**
...
- [ ] **Step 6: `llms.txt` agrees with `DATA`** — name, role, both approved metric values, and
the publication count. This is what actually prevents the drift Task 2 could not.

- [ ] **Step 7: Mutation-test every category.** Break each one deliberately — wrong declared
...
- [ ] **Step 1:** Gate every contract, inspect all six diffs (expect a change in the hero corners
from the clip, and nothing else), regenerate the baseline, `verify.sh` exits 0.
- [ ] **Step 2: One commit**, then push and confirm the deploy.
- [ ] **Step 3: Verify on the real URL** — all three artefacts return 200; `og.png` returns 200 and
is 1200×630; canonical present and correct; JSON-LD present.
- [ ] **Step 4: Check the card as a scraper sees it** — fetch the deployed HTML and confirm the OG
